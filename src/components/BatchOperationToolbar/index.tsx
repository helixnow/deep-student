...
/>
            {searchQuery && (
              <DsButton variant="ghost" size="icon" iconOnly className="clear-search [@media(pointer:coarse)]:!min-h-11 [@media(pointer:coarse)]:!min-w-11" onClick={() => setSearchQuery('')} aria-label="clear">
                <X size={14} />
              </DsButton>
            )}
          </div>
        ...
        <div className="toolbar-section batch-actions">
          ...
          <div className="dropdown-container" ref={moreMenuRef}>
            <DsButton variant="ghost" size="icon" iconOnly className="action-btn more [@media(pointer:coarse)]:!min-w-11" onClick={() => setShowMoreMenu(!showMoreMenu)} aria-label="more" aria-expanded={showMoreMenu}>
