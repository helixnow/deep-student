 ...
 * - 章节一致 → 本机状态（保留章内滚动位置）；
 * - 章节不一致 → 时间戳更新的一侧赢；metadata 赢时从章首开始。
 *   平局（都为 0，如旧 payload + 无 lastReadAt 的 metadata）时 metadata 赢：
 *   它是跨设备可靠通道，本机旧 payload 无法证明自己更新。
 */
export function resolveInitialEpubLocation(
  ...
  const metaAt = metadataProgress?.lastReadAt || 0;
  if (metaAt >= localAt) {
