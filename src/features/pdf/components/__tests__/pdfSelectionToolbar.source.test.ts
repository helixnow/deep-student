...
describe('EnhancedPdfViewer mounts the shared selection toolbar', () => {
  it('renders PdfSelectionActions inside the viewer root', () => {
    expect(viewerSource).toContain("import { PdfSelectionActions } from './PdfSelectionActions'");
...
describe('PdfSelectionActions reuses existing capabilities', () => {
  ...
  it('routes cards through the existing selectionCardGeneration pipeline', () => {
  ...
  it('reuses chat explain / translate popovers instead of new result UI', () => {
    expect(actionsSource).toContain("from '@/features/chat/components/ExplainPopover'");
    expect(actionsSource).toContain("from '@/features/chat/components/TranslationPopover'");
  });
});
