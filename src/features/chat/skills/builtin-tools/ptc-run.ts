/**
 * PTC 程序化工具组合技能（G05-P1/P2/P3）
 *
 * 允许 AI agent 提交一段 Starlark 脚本，在脚本内通过 `call(tool, args)`
 * 串行组合多个受控内置工具调用（循环 / 分支 / 聚合中间结果），只把最终
 * 答案交还给模型，避免多轮工具往返撑爆上下文。
 *
 * 每个 `call` 都重新经过中央准入（kill-switch / 白名单 / 审批），工具面
 * 为注册表驱动的只读白名单 + 极小非破坏性写面；shell、connector、子代理
 * 及删除/移动类写工具一律 fail-closed 拒绝。
 *
 ...
 *
 * G05-P3 新增 `object_write` 与五个受控写工具；所有写仍逐次经过中央准入，
 * trace 标记 side_effect，并汇总到 writes_summary 供验收。
 *
  ...
  embeddedTools: [
    {
      ...
      description:
        'Runs a Starlark script through the Rust backend executor, composing registry-allowlisted reads and a small controlled write surface. Central admission is re-applied to every call; object_read/object_write access only managed artifacts.',
      inputSchema: {
        type: 'object',
        properties: {
          script: {
            type: 'string',
            description:
              'Starlark script source. The last expression is returned. Use call(tool, args) for allowlisted tools, object_read(...) to page materialized objects, and object_write(...) for controlled writes under the artifacts root.',
            maxLength: 65536,
          },
          max_calls: {
            type: 'integer',
