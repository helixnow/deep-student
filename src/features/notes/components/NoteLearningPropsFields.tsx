...
export function NoteLearningPropsFields({ value, disabled, readOnly, onSave }: {
  ...
  const save = async () => {
    try {
      const next = updateNoteLearningProps(value, changes);
      if (await onSave(next)) setChanges({});
    } catch (cause) {
      setError(cause instanceof Error ? cause.message : String(cause));
    }
  };
  return (
    <fieldset className="my-3 space-y-2 text-xs" disabled={disabled || readOnly}>
          ...
          <label key={field} className="block space-y-1">
            ...
            {field === 'mastery' ? (
              <select className="w-full rounded border border-border bg-background p-1.5" value={inputValue}
                onChange={(event) => { setChanges((prev) => ({ ...prev, [field]: event.target.value })); setError(''); }}>
      ...
      {!readOnly && <LearningTemplatePresetPicker course={changes.course ?? current.course ?? ''} disabled={disabled}
        onChoose={(preset) => setChanges((draft) => {
          const next = { ...draft };
