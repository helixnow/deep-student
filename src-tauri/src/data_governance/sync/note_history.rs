...
use crate::vfs::repos::{
    note_format_repo::{NoteFormat, NoteFormatRepo},
...
fn error(e: impl std::fmt::Display) -> SyncError {
    SyncError::Database(e.to_string())
}

pub(super) fn include_note_dependencies(
    conn: &Connection,
    changes: &mut Vec<SyncChangeWithData>,
) -> Result<(), SyncError> {
    if !SyncManager::table_has_column(conn, "note_document_revisions", "version_id") {
        return Ok(());
    }
    let mut seen: HashSet<_> = changes
        .iter()
        .map(|c| (c.table_name.clone(), c.record_id.clone()))
        .collect();
    let notes: Vec<_> = changes
        .iter()
        .filter(|c| {
            c.table_name == "notes" && c.operation != ChangeOperation::Delete && c.data.is_some()
        })
        .cloned()
        .collect();
    for note in notes {
        for (table, key) in [
            ("note_document_formats", "note_id"),
            ("note_document_revisions", "version_id"),
            ("note_learning_relations", "id"),
        ] {
            if !SyncManager::table_has_column(conn, table, key) {
                continue;
            }
            let order = if table == "note_document_revisions" {
                "seq"
            } else {
                "note_id"
            };
            let mut stmt = conn
                .prepare(&format!(
                    "SELECT {key} FROM {table} WHERE note_id=?1 ORDER BY {order}"
                ))
                .map_err(error)?;
            let ids = stmt
                .query_map([&note.record_id], |r| r.get::<_, String>(0))
                .map_err(error)?;
            for id in ids {
                let id = id.map_err(error)?;
                if !seen.insert((table.into(), id.clone())) {
                    continue;
                }
...
pub(super) fn prepare_relation(
    ...
    let locator: NoteLocator = serde_json::from_str(
        obj.get("locator_json")
            .and_then(Value::as_str)
            .ok_or_else(|| error("Relation locator missing"))?,
    )
    .map_err(error)?;
    let kind = obj
        .get("relation_type")
        .and_then(Value::as_str)
        .ok_or_else(|| error("Relation type missing"))?;
    ...
    let current: Option<i64> = conn
        .query_row(
            "SELECT revision FROM note_learning_relations WHERE id=?1",
            [id],
            |r| r.get(0),
        )
        .optional()
    ...
    let revision = current
        .unwrap_or(0)
        .checked_add(1)
        .ok_or_else(|| error("Relation revision exhausted"))?;
    obj.insert("revision".into(), Value::from(revision));
    Ok(())
}

pub(super) fn retarget_note_relations(
    conn: &Connection,
    note_id: &str,
    previous: &str,
) -> Result<(), SyncError> {
    if !SyncManager::table_has_column(conn, "note_learning_relations", "revision") {
        return Ok(());
    }
    let (resource, updated): (String, String) = conn
        .query_row(
            "SELECT resource_id,updated_at FROM notes WHERE id=?1",
            [note_id],
    ...
    let active: bool = conn
        ...
        .map_err(error)?;
    if active {
        crate::vfs::repos::note_relation_repo::NoteRelationRepo::invalidate_missing_blocks(
            conn, note_id,
        )
        .map_err(error)?;
    }
...
pub(super) fn apply_revision(conn: &Connection, id: &str, data: &Value) -> Result<(), SyncError> {
    let mut obj = data
        .as_object()
        .ok_or_else(|| error("Invalid history object"))?
        .clone();
    if obj.get("version_id").and_then(Value::as_str) != Some(id) {
        return Err(error("History version identity mismatch"));
    }
    ...
    for key in ["seq", "pinned", "edit_bucket", super::SYNC_FIELD_DELTAS_KEY] {
        obj.remove(key);
    }
    for key in [
        "note_id",
        ...
        if obj.get(key).and_then(Value::as_str).is_none() {
            return Err(error(format!("History field missing: {key}")));
        }
    }
    let format = NoteFormat {
        note_id: obj["note_id"].as_str().unwrap().into(),
        content_format: obj["content_format"].as_str().unwrap().into(),
        format_version: obj
            .get("format_version")
            .and_then(Value::as_i64)
            .ok_or_else(|| error("History format version missing"))?,
        serializer_version: obj["serializer_version"].as_str().unwrap().into(),
        baseline_version_id: None,
    };
    NoteFormatRepo::ensure_supported(&format).map_err(error)?;
    let detected = NoteFormatRepo::detect(&format.note_id, obj["content_md"].as_str().unwrap())
        .map_err(error)?;
    if detected.content_format != format.content_format {
        return Err(error("History format/body mismatch"));
    }
    ...
    serde_json::from_str::<Vec<crate::vfs::repos::note_revision_repo::NoteAssetRef>>(
        obj["asset_refs_json"].as_str().unwrap(),
    )
    .map_err(error)?;
    if let Some(s) = obj.get("props_json").and_then(Value::as_str) {
        serde_json::from_str::<Value>(s).map_err(error)?;
    }
    if let Some(local) =
        SyncManager::get_record_data(conn, "note_document_revisions", id, "version_id")?
    {
        let local = local
            .as_object()
            .ok_or_else(|| error("Invalid local history"))?;
        for (key, value) in &obj {
            let equal = if key.ends_with("_json") {
                ...
                local.get(key).unwrap_or(&Value::Null) == value
            };
            if !equal {
                return Err(error(format!("Immutable history conflict: {id} ({key})")));
            }
        }
        if incoming_pin {
            conn.execute(
                "UPDATE note_document_revisions SET pinned=1 WHERE version_id=?1 AND pinned=0",
                [id],
            )
            .map_err(error)?;
        }
    ...
    conn.execute(
        &format!("INSERT INTO note_document_revisions ({columns}) VALUES ({placeholders})"),
        params.as_slice(),
    )
    .map_err(error)?;
    Ok(())
...
pub(super) fn snapshot_notes(
    ...
    for id in ids {
        if source == "remote_sync" {
            ...
            if !NoteFormatRepo::required_capabilities(&old).is_empty()
                && NoteFormatRepo::required_capabilities(&format).is_empty()
            {
                format.serializer_version =
                    NoteFormatRepo::serializer(format.content_format == "markdown-blocks", true);
            }
            if old.content_format != format.content_format
                || old.format_version != format.format_version
                || old.serializer_version != format.serializer_version
            {
                ...
                conn.execute(
                    "UPDATE note_document_formats SET updated_at='' WHERE note_id=?1",
                    [id],
                )
                .map_err(error)?;
            }
        ...
        if source == "before_sync" {
            NoteRevisionRepo::set_pinned(conn, id, &version, true).map_err(error)?;
        }
    }
    Ok(())
}
...
pub(super) fn validate_batch_formats(changes: &[SyncChangeWithData]) -> Result<(), SyncError> {
    ...
    for change in changes.iter().filter(|c| {
        c.table_name == "note_document_formats" && c.operation != ChangeOperation::Delete
    }) {
        let format: NoteFormat = serde_json::from_value(
            change
                .data
                .clone()
                .ok_or_else(|| error("Missing note format envelope"))?,
        )
        .map_err(error)?;
        NoteFormatRepo::ensure_supported(&format).map_err(error)?;
        if format.note_id != change.record_id {
            return Err(error("Note format identity mismatch"));
        }
        let resource_id = changes
            .iter()
            .find(|c| c.table_name == "notes" && c.record_id == format.note_id)
            .and_then(|c| c.data.as_ref())
            .and_then(|d| d.get("resource_id"))
            .and_then(Value::as_str);
        if let Some(body) = resource_id
            .and_then(|id| {
                changes
                    .iter()
                    .find(|c| c.table_name == "resources" && c.record_id == id)
            })
            .and_then(|c| c.data.as_ref())
            ...
            .and_then(Value::as_str)
        {
            ...
            if detected.content_format != format.content_format
                || (!NoteFormatRepo::required_capabilities(&detected).is_empty()
                    && NoteFormatRepo::required_capabilities(&format).is_empty())
            {
...
pub(super) fn validate_incoming(
    conn: &Connection,
    table: &str,
    id: &str,
    data: &Value,
    ...
    if table == "resources"
        && (data.get("type").and_then(Value::as_str) == Some("note")
            || !affected_notes(conn, table, id)?.is_empty())
    {
        if let Some(body) = data.get("data").and_then(Value::as_str) {
            NoteFormatRepo::detect(id, body).map_err(error)?;
        }
    }
...
mod tests {
    use super::*;
    use crate::vfs::{
        database::setup_migrated_test_db, VfsCreateNoteParams, VfsNoteRepo, VfsUpdateNoteParams,
    };
    ...
    fn direct_remote_resource_update_keeps_both_full_document_snapshots() {
        ...
        let note = VfsNoteRepo::create_note(
            &source,
            VfsCreateNoteParams {
                title: "Resource fixture".into(),
                content: "original full document\n\nsuffix".into(),
                tags: vec![],
            },
        )
        .unwrap();
        ...
        assert_eq!(
            SyncManager::apply_downloaded_changes(&target_conn, &changes, None)
                .unwrap()
                .failure_count,
            0
        );
        let mut resource = changes
            .into_iter()
            .find(|c| c.table_name == "resources" && c.record_id == note.resource_id)
            .unwrap();
        resource.data.as_mut().unwrap()["data"] =
            Value::from("remote full document\n\nremote suffix");
        resource.data.as_mut().unwrap()["updated_at"] =
            Value::from(chrono::Utc::now().timestamp_millis());
        resource.operation = ChangeOperation::Update;
        let result =
            SyncManager::apply_downloaded_changes(&target_conn, &[resource], None).unwrap();
        assert_eq!(result.failure_count, 0, "{result:?}");
        assert_eq!(
            VfsNoteRepo::get_note_content_with_conn(&target_conn, &note.id)
                .unwrap()
                .unwrap(),
        ...
        let snapshots: Vec<_> = versions
            .items
            .iter()
            .map(|v| NoteRevisionRepo::get(&target_conn, &note.id, &v.version_id).unwrap())
            .collect();
        assert!(snapshots
            .iter()
            .any(|v| v.content_md == "original full document\n\nsuffix" && v.summary.pinned));
        assert!(snapshots
            .iter()
            .any(|v| v.content_md == "remote full document\n\nremote suffix"));
    }
    ...
    fn relation_sync_rebuilds_local_cas_and_replay_does_not_increment_it() {
        ...
        let changes = payloads(&source_conn);
        let mut relation = changes
            .iter()
            .find(|c| c.table_name == "note_learning_relations")
            .unwrap()
            .clone();
        ...
        let revision = || {
            target_conn
                .query_row(
                    "SELECT revision FROM note_learning_relations WHERE id='relation'",
                    [],
                    |r| r.get::<_, i64>(0),
                )
                .unwrap()
        ...
        assert_eq!(
            target_conn
                .query_row(
                    "SELECT count(*) FROM resources WHERE id='missing-target'",
                    [],
                    |r| r.get::<_, i64>(0)
                )
                .unwrap(),
        ...
        relation.data.as_mut().unwrap()["updated_at"] =
        ...
        assert_eq!(revision(), 2);
        let row = crate::vfs::repos::note_relation_repo::NoteRelationRepo::get(
            &target_conn,
            None,
            "relation",
        )
    ...
    fn remote_pin_is_retained_after_unpinned_replay_and_prune() {
        ...
        let original = payloads(&conn)
            .into_iter()
            .find(|c| c.table_name == "note_document_revisions")
            .unwrap();
        NoteRevisionRepo::set_pinned(&conn, &note.id, &original.record_id, false).unwrap();
        let mut pinned = original.clone();
        pinned.data.as_mut().unwrap()["pinned"] = Value::from(1);
        assert_eq!(
            SyncManager::apply_downloaded_changes(&conn, &[pinned], None)
                .unwrap()
                .failure_count,
            0
        );
        let mut unpinned = original.clone();
        unpinned.data.as_mut().unwrap()["pinned"] = Value::from(0);
        assert_eq!(
            SyncManager::apply_downloaded_changes(&conn, &[unpinned], None)
                .unwrap()
                .failure_count,
            0
        );
        let mut prune = original.clone();
        prune.operation = ChangeOperation::Delete;
        prune.data = None;
        SyncManager::apply_downloaded_changes(&conn, &[prune], None).unwrap();
        assert!(
            NoteRevisionRepo::get(&conn, &note.id, &original.record_id)
                .unwrap()
                .summary
                .pinned
        );
    ...
    fn row_sync_transmits_history_dependencies_and_preserves_remote_before_after() {
        ...
        future_batch
            .iter_mut()
            .find(|c| c.table_name == "note_document_formats")
            .unwrap()
            .data
            .as_mut()
            .unwrap()["format_version"] = Value::from(999);
        ...
        future_capability
            ...
            .data
            .as_mut()
            .unwrap()["serializer_version"] = Value::from("markdown-v1+ds-columns-v999");
        assert!(
            SyncManager::apply_downloaded_changes(&target_conn, &future_capability, None).is_err()
        );
        assert_eq!(
            target_conn
        ...
        .unwrap();
        let result =
            SyncManager::apply_downloaded_changes(&target_conn, payloads(&source_conn), None)
                .unwrap();
        assert_eq!(result.failure_count, 0, "{result:?}");
        assert_eq!(
            VfsNoteRepo::get_note_content_with_conn(&target_conn, &note.id)
        ...
        let states: Vec<(String, bool)> = target_conn
            .prepare("SELECT content_md,pinned FROM note_document_revisions WHERE note_id=?1")
            .unwrap()
            .query_map([&note.id], |r| Ok((r.get(0)?, r.get(1)?)))
            .unwrap()
            .collect::<rusqlite::Result<_>>()
            .unwrap();
        assert!(states
            .iter()
            .any(|(body, pinned)| body == "before" && *pinned));
        assert!(states.iter().any(|(body, _)| body == "after"));
        let restored =
            NoteRevisionRepo::restore_copy(&target_conn, &note.id, &history.record_id).unwrap();
        assert_eq!(
            VfsNoteRepo::get_note_content_with_conn(&target_conn, &restored.id)
                .unwrap()
                .unwrap(),
            "before"
        );
    ...
    fn immutable_history_conflict_is_quarantined_and_valid_retry_preserves_local_pins() {
        ...
        assert_eq!(saved.content_md, "original");
        assert!(saved.summary.pinned);
        let result =
            SyncManager::apply_downloaded_changes(&conn, &[original.clone()], None).unwrap();
        assert_eq!(result.failure_count, 0);
        let mut deletion = original.clone();
        deletion.operation = ChangeOperation::Delete;
        deletion.data = None;
        ...
        future.record_id = "future-version".into();
        let data = future.data.as_mut().unwrap();
        data["version_id"] = Value::from("future-version");
        data["format_version"] = Value::from(999);
        assert_eq!(
            SyncManager::apply_downloaded_changes(&conn, &[future], None)
                .unwrap()
                .failure_count,
            1
        );
