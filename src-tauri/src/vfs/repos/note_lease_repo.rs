...
impl NoteLeaseRepo {
    ...
    pub fn status(conn: &Connection, token: &str) -> VfsResult<Option<LeaseStatus>> {
        ...
        let Some((operation_id, owner_id, phase, expires_at)) = row else {
            return Ok(None);
        };
        ...
        let waiting_for = stmt
            .query_map(params![token, phase], |r| r.get(0))?
            .collect::<rusqlite::Result<Vec<_>>>()?;
        Ok(Some(LeaseStatus {
            token: token.into(),
            operation_id,
    ...
    fn required(conn: &Connection, token: &str, now: i64) -> VfsResult<LeaseStatus> {
        ...
        if status.expires_at <= now || dead {
            return Err(conflict("notes.lease_expired"));
        }
        Ok(status)
    }
    ...
    fn owned(
        ...
        webview: &str,
        now: i64,
    ) -> VfsResult<LeaseStatus> {
        Self::participant(conn, &auth.participant_id, webview, now)?;
        let status = Self::required(conn, &auth.token, now)?;
        if status.owner_id != auth.participant_id {
            return Err(conflict("notes.lease_owner_mismatch"));
        }
        Ok(status)
    }
    ...
    pub fn register(
        ...
        window: &str,
        now: i64,
    ) -> VfsResult<ParticipantStatus> {
        NoteRevisionRepo::transaction(conn, || {
            if VfsNoteRepo::get_note_with_conn(conn, note_id)?.is_none() {
                return Err(invalid("Note missing"));
            }
            let id = format!("nep_{}", uuid::Uuid::new_v4().simple());
            conn.execute(
                "INSERT INTO note_editor_participants VALUES(?1,?2,?3,?4,?5)",
                params![id, webview, window, note_id, now + PARTICIPANT_TTL],
            )?;
            let token: Option<String> = conn
                .query_row(
                    "SELECT token FROM note_editor_lease_notes WHERE note_id=?1",
                    [note_id],
                    |r| r.get(0),
                )
                .optional()?;
            let active_lease = if let Some(token) = token {
                conn.execute(
                    "INSERT INTO note_editor_lease_acks(token,participant_id) VALUES(?1,?2)",
                    params![token, id],
                )?;
            ...
            Ok(ParticipantStatus {
                participant_id: id,
                expires_at: now + PARTICIPANT_TTL,
                active_lease,
            })
        })
    }
    ...
    pub fn heartbeat(
        ...
        NoteRevisionRepo::transaction(conn, || {
            ...
            conn.execute(
                "UPDATE note_editor_participants SET expires_at=?2 WHERE id=?1",
                params![id, now + PARTICIPANT_TTL],
            )?;
            ...
            Ok(ParticipantStatus {
                ...
                expires_at: now + PARTICIPANT_TTL,
                active_lease: token.map(|t| Self::status(conn, &t)).transpose()?.flatten(),
            })
        })
    }
    ...
    pub fn begin(
        ...
        NoteRevisionRepo::transaction(conn, || {
            ...
            if ids.is_empty() || operation_id.trim().is_empty() {
                return Err(invalid("Operation and note IDs required"));
            }
            if let Some(token) = conn
                .query_row(
                    "SELECT token FROM note_editor_leases WHERE operation_id=?1",
                    [operation_id],
                    |r| r.get::<_, String>(0),
                )
                .optional()?
            {
                let status = Self::owned(
                    conn,
                    &NoteLeaseAuth {
                ...
                if status
                    ...
                    != ids
                {
                    return Err(conflict("notes.lease_operation_mismatch"));
                }
                return Ok(status);
            }
            let token = format!("nel_{}", uuid::Uuid::new_v4().simple());
            conn.execute(
                "INSERT INTO note_editor_leases VALUES(?1,?2,?3,'pending',?4)",
                params![token, operation_id, owner, now + LEASE_TTL],
            )?;
            for id in ids {
                if VfsNoteRepo::get_note_with_conn(conn, &id)?.is_none() {
                    return Err(invalid("Note missing"));
                }
                if conn.query_row(
                    "SELECT EXISTS(SELECT 1 FROM note_editor_lease_notes WHERE note_id=?1)",
                    [&id],
                    |r| r.get::<_, bool>(0),
                )? {
                    return Err(conflict("notes.lease_conflict"));
                }
                conn.execute(
                    "INSERT INTO note_editor_lease_notes VALUES(?1,?2)",
                    params![id, token],
                )?;
    ...
    pub fn ack(
        ...
        NoteRevisionRepo::transaction(conn, || {
            ...
            let status = Self::required(conn, &auth.token, now)?;
            if status.phase != "pending" && status.phase != "ready" {
                return Err(conflict("notes.lease_phase"));
            }
            let current = VfsNoteRepo::get_note_with_conn(conn, &note_id)?
                .ok_or_else(|| invalid("Note missing"))?;
            if draft.expected_updated_at != current.updated_at {
                return Err(conflict("notes.lease_stale_draft"));
            }
            ...
            let previous = stmt
                .query_map(params![auth.token, note_id], |r| r.get::<_, String>(0))?
                .collect::<rusqlite::Result<Vec<_>>>()?;
            for raw in previous {
                let previous: FrozenDraft =
                    serde_json::from_str(&raw).map_err(|e| invalid(&e.to_string()))?;
                if previous != draft {
                    return Err(conflict("notes.lease_divergent_drafts"));
                }
            }
    ...
    pub fn authorized<T>(
        ...
        now: i64,
        commit: bool,
        work: impl FnOnce() -> VfsResult<T>,
    ) -> VfsResult<T> {
        NoteRevisionRepo::transaction(conn, || {
            // Acquire SQLite's writer lock before inspecting the barrier.
            conn.execute(
                "INSERT INTO note_editor_write_grants VALUES(?1)",
                [&auth.token],
            )?;
            let status = Self::owned(conn, auth, webview, now)?;
            if status.phase != "ready" {
                return Err(conflict("notes.lease_not_ready"));
            }
            if commit {
                ...
                for (id, raw) in drafts {
                    ...
                    if current.content_md != draft.markdown
                        || current.updated_at != draft.expected_updated_at
                    {
                        return Err(conflict("notes.lease_flush_required"));
                    }
                }
            }
            let result = work()?;
            ...
            conn.execute(
                "DELETE FROM note_editor_write_grants WHERE token=?1",
                [&auth.token],
            )?;
            Ok(result)
        })
    ...
    pub fn finish(
        ...
        NoteRevisionRepo::transaction(conn, || {
            ...
            conn.execute(
                "UPDATE note_editor_leases SET phase='refreshing',expires_at=?2 WHERE token=?1",
                params![auth.token, now + LEASE_TTL],
            )?;
            Self::required(conn, &auth.token, now)
        })
    }
    ...
    pub fn flush(
        ...
        Self::authorized(conn, auth, webview, now, false, || {
            ...
            let Some(raw) = raw else {
                if !Self::required(conn, &auth.token, now)?
                    .notes
                    .iter()
                    .any(|note| note.note_id == note_id)
                {
                    return Err(conflict("notes.lease_scope"));
                }
            ...
            let mut draft: FrozenDraft =
                serde_json::from_str(&raw).map_err(|e| invalid(&e.to_string()))?;
            let current = NoteRevisionRepo::current(conn, note_id)?;
            let note = if current.content_md == draft.markdown
                && current.updated_at == draft.expected_updated_at
            {
                VfsNoteRepo::get_note_with_conn(conn, note_id)?
                    .ok_or_else(|| invalid("Note missing"))?
            } else {
                VfsNoteRepo::update_note_with_capabilities(
                    ...
                    crate::vfs::types::VfsUpdateNoteParams {
                        content: Some(draft.markdown.clone()),
                        expected_updated_at: Some(draft.expected_updated_at.clone()),
                        ..Default::default()
                    },
                    capabilities,
                )?
    ...
    pub fn refresh_ack(
        ...
        NoteRevisionRepo::transaction(conn, || {
            ...
            if !status
                .notes
                .iter()
                .any(|n| n.note_id == note_id && n.updated_at == updated_at)
            {
                return Err(conflict("notes.lease_stale_refresh"));
            }
    ...
    pub fn release(
        ...
        cancel: bool,
        now: i64,
    ) -> VfsResult<LeaseCancellation> {
        NoteRevisionRepo::transaction(conn, || {
            let status = Self::owned(conn, auth, webview, now)?;
            if !cancel && (status.phase != "refreshing" || !status.waiting_for.is_empty()) {
                return Err(conflict("notes.lease_refresh_pending"));
            }
            Self::remove(
                conn,
                &auth.token,
    ...
    pub(crate) fn remove(
        ...
        conn.execute(
            "DELETE FROM note_editor_lease_notes WHERE token=?1",
            [token],
        )?;
        conn.execute(
            "DELETE FROM note_editor_write_grants WHERE token=?1",
        ...
        Ok(LeaseCancellation {
            token: token.into(),
            note_ids: status
                .map(|s| s.notes.into_iter().map(|n| n.note_id).collect())
                .unwrap_or_default(),
            reason: reason.into(),
        })
    }
    ...
    pub fn cleanup(
        ...
        window: Option<&str>,
        webview: Option<&str>,
    ) -> VfsResult<Vec<LeaseCancellation>> {
        NoteRevisionRepo::transaction(conn, || {
            ...
            tokens
                .iter()
                .map(|t| Self::remove(conn, t, "participant_closed_or_expired"))
                .collect()
        })
    }
    ...
    pub fn unregister(
        ...
        NoteRevisionRepo::transaction(conn, || {
            conn.execute(
                "DELETE FROM note_editor_participants WHERE id=?1 AND webview_label=?2",
                params![id, webview],
            )?;
