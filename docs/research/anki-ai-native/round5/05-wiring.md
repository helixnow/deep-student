...
| Sidekick routing | Generator 在 streaming 路径消费路由；当前 HEAD 的 Critic 也出现了 `resolve_anki_role_decision` / `call_anki_routed_raw_prompt` 调用 | 本轮不扩展路由 | **并非“只用于 Generator”**：Generator 已接；Critic 有调用点。但当前 HEAD 未定义上述两个 `LLMManager` 方法，因此 Critic 属于不完整接线，不能据此宣称已可用；Planner/Vlm 角色仍未消费 |
