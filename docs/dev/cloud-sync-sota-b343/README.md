...
| 收尾 | 进行中 | P2-1/P2-2/P2-6 稳定 code 已关；delta 积木已合（未接线）；中性对象名阶段一已合；记录级/tombstone 路径短哈希已合；文件级清单与快照新写入改为中性 UUID；资产 tombstone 显式 skip 共享对象已合（未带 ftp.rs）；见 [WRAP-CLOSE.md](./WRAP-CLOSE.md) |
