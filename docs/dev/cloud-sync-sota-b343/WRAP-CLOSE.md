...
- **增量备份**：`DELTA-R11.md` 已合；codec + inventory + backup-v2 租约 + `publish_verified_staging` + `restore_snapshot_to_staging` 已落。**未接线**：生产仍整 ZIP 单对象 PUT，不能宣称增量/去重/CDC。下一刀 GC，最后才 integration。
