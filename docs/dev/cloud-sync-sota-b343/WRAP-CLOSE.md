...
- **CI / Rust 门禁**：`c06a7959` 的 Frontend（licenses + tsc）、Backend、Migration Gate、Cloud Provider Contract Gate 已过。Vitest 分片曾把单个 worker 顶死在 `max-old-space-size=4096`（日志约 4001MB，无断言失败）。CI 现为 6144MB + `maxForks: 2`，不放宽 autosync/StatusBar 用例。Dashboard 本地 ZIP 导入测已对齐「先确认可选密码对话框」；导出断言含第 5 个 `encryptionPassword` 参数。本地 ZIP 密码按 Unicode 标量计数（与后端 `chars().count()` 对齐），单项导出也走同一校验。CLAAssistant 忽略。完整 CI 未宣称全绿。
- **供应商兼容**：已移植 #174——WebDAV href/base 统一解码（坚果云中文/空格路径不再静默列空）；S3 `normalize_endpoint` 剥离控制台 bucket 前缀域名/路径后缀。未带更松的 `ftp.rs` 550 白名单。`localizeCloudStorageError` 由云设置页与数据治理本地 ZIP 导入/导出共用，将「已开启 E2EE 但读不到已存密码」与 `Missing WebDAV/S3/FTP configuration` 映射为 zh/en 人话，不改 `toSafeCloudStorageConfig` 的英文抛错契约。
