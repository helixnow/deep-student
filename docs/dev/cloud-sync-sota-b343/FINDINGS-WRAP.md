...
| §3.1 增量去重 | **调研+codec 已合，生产未接线** | [DELTA-R11.md](./DELTA-R11.md) + `delta_format.rs`；整 ZIP 仍整对象 PUT；不能宣称增量/去重/CDC |
...
**生产放量仍 NO-GO。** 当前最短剩余：完整 CI 绿灯、Android 真机签字、整包增量传输接线（inventory/upload/restore，codec 不算实现）。P2-1/P2-2 已合。
