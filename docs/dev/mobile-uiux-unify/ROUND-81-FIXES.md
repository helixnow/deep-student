# Round 81 落地（claude-fable-5-thinking-xhigh）

## 已修

- ExposeOverlay 关闭钮 22px + hover-only（触屏常显并 44）
- ShortcutCheatsheet 关闭钮 24px
- DesktopAgendaWidget 头栏图标 / 待办勾选 coarse 38→44
- SkillsManagementPage 位置筛选 SegmentedControl：`isSmallScreen` 桌面支 `!py-1` 在 iPad 横屏 coarse 无 44（补 `!min-h-11` 压过 `.study-shell-segmented-button { min-height: 0 }`）
- TodoQuickAdd 优先级分段 `!py-1`（同上 `!min-h-11`）
- TodoItemDetail 优先级 / 重复分段 `!py-0.5`（同上）
- PomodoroPanel 环境音分段（设置区 + 内联行）
- PomodoroWindowSettings 环境音分段
- quick-assistant 图标钮 / 返回 / 保存行 / 菜单项 / 清除（标题栏 coarse 升到 44 以免裁切）
- Crepe 块手柄 `.operation-item` 26px `!important`（slash/块菜单已有 coarse 未动）
