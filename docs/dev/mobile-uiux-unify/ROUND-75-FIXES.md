# Round 75 落地（claude-fable-5-thinking-xhigh）

## 已修

- shad Input（`lg:min-h-[var(--button-height)]` 无 coarse，设置/搜索/表单共用；保留 `lg:min-h` 以满足 migrationFoundation 断言）
- EmbeddedToolsEditor 工具名 Input（`h-8 max-lg:h-11` 无 coarse）及折叠触发器
- shad Checkbox.css（视觉 16×16，伪元素扩 coarse 命中，学 Switch.css）
- shad Slider 拇指（视觉 12×12，`after:-inset-[16px]` 扩 coarse 命中）
- api-key-field.css（`min-width: 1024px` 缩到 `--button-height` 无 coarse 还原）
- TodoQuickAdd 到期日 Input / 包裹层（`min-h-0` + `py-1` 取消 44）
- SkillsManagementPage 列表搜索（`isSmallScreen ? h-11 : h-8` iPad 洞）
- Anki SessionRow 展开主行（仅 `<768` 有 `min-h-[44px]`，iPad coarse 仍 ~36）
- shad Button.css `data-size="icon"`（`min-width: 1024px` 缩到 `--button-icon-size` 无 coarse 还原）
- TodoItemRow 行内改期 date Input（`min-h-0` 取消共享 Input 44）
