...
28. ~~**Round 74 CI worker 堆**~~ — ✅ forks `execArgv` 6144 + Windows UA stub + scrollbar 扫描超时
...
33. ~~**Round 79 分片 4 合同**~~ — ✅ restoreSession 默认关闭 / 空闲超时失败 / PDF 缺省不带图
34. **桌面手动 smoke**（可选）— 见 [TAURI_E2E.md](./TAURI_E2E.md)
...
| 多轮迭代至 SOTA | Round 6–79 本地完成；合入 main 仍待 CI + 批准 | ⏳ 进行中（不标 complete） |
